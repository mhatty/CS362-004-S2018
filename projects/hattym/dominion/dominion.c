...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      adventuererEffect(state, currentPlayer);
      return 0;
			
    case council_room:
      councilEffect(state, currentPlayer, handPos);		
      return 0;
...
void adventuererEffect(struct gameState *state, int currentPlayer)
{
	int z = 0;
	int cardDrawn;
	int drawntreasure = 0; 
...
void councilEffect(struct gameState *state, int currentPlayer, int handPos)
{
	int i;
	//+4 Cards
    for (i = 0; i < 4; i++)
